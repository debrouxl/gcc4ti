...
!define PRODUCT_VERSION "0.96 Beta 11"
...
; Other definitions
!define TIGCCDIR "C:\gcc4ti"
!define TIGCCCVSDIR "C:\gcc4ti"
...
!define MUI_STARTMENUPAGE_DEFAULTFOLDER "GCC4TI SDK"
!define MUI_STARTMENUPAGE_REGISTRY_ROOT HKLM
!define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\GCC4TI Team\GCC4TI"
...
OutFile "Setup.exe"
InstallDir "$PROGRAMFILES\GCC4TI"
...
noseresoft:
  ReadRegStr $0 HKLM "Software\GCC4TI Team\GCC4TI" "Program Folder"
  StrCmp $0 "" nogcc4titeam
  StrCpy $INSTALLED_BEFORE 1
  StrCpy $INSTDIR $0
  ReadRegStr $0 HKLM "Software\GCC4TI Team\GCC4TI" "Program Group"
  StrCmp $0 "" nogcc4titeam
  StrCpy $ICONS_GROUP $0
nogcc4titeam:
...
Section "Core GCC4TI Components" SEC01
  SectionIn 1 RO
; delete obsolete files
  Delete "$SYSDIR\GCC4TIUninst.exe"
  GetFullPathName $0 "."
...
; delete obsolete registry keys
  ...
  DeleteRegValue HKLM "Software\GCC4TI Team\GCC4TI" "Program Group Existed"
; install files
  SetOutPath "$INSTDIR\Bin"
  File "${TIGCCDIR}\Bin\m68k-coff-tigcc-as.exe"
  File "${TIGCCDIR}\Bin\cc1.exe"
  File "${TIGCCDIR}\Bin\m68k-coff-tigcc-gcc.exe"
  File "${TIGCCDIR}\Bin\m68k-coff-tigcc-objcopy.exe"
  File "${TIGCCDIR}\Bin\m68k-coff-tigcc-objdump.exe"
  File "${TIGCCDIR}\Bin\m68k-coff-tigcc-size.exe"
  File "${TIGCCDIR}\Bin\m68k-coff-tigcc-strip.exe"
  File "${TIGCCDIR}\Bin\ar-tigcc.exe"
  File "${TIGCCDIR}\Bin\ld-tigcc.exe"
  File "${TIGCCDIR}\Bin\link.dll"
  File "${TIGCCDIR}\Bin\tigcc-patcher.exe"
  File "${TIGCCDIR}\Bin\ttarchive.exe"
  File "${TIGCCDIR}\Bin\ttbin2bin.exe"
  File "${TIGCCDIR}\Bin\ttbin2hex.exe"
  File "${TIGCCDIR}\Bin\ttbin2oth.exe"
  File "${TIGCCDIR}\Bin\ttbin2str.exe"
  File "${TIGCCDIR}\Bin\ttchecksum.exe"
  File "${TIGCCDIR}\Bin\ttextract.exe"
  File "${TIGCCDIR}\Bin\tthelp.exe"
  File "${TIGCCDIR}\Bin\tthex2bin.exe"
  File "${TIGCCDIR}\Bin\ttinfo.exe"
  File "${TIGCCDIR}\Bin\ttpack.exe"
  File "${TIGCCDIR}\Bin\ttppggen.exe"
  File "${TIGCCDIR}\Bin\ttsetname.exe"
  File "${TIGCCDIR}\Bin\ttsplit.exe"
  File "${TIGCCDIR}\Bin\ttstrip.exe"
  File "${TIGCCDIR}\Bin\tttiler.exe"
  File "${TIGCCDIR}\Bin\ttunarchive.exe"
  File "${TIGCCDIR}\Bin\ttunpack.exe"
  SetOutPath "$INSTDIR\Lib"
  File "${TIGCCDIR}\Lib\tigcc.a"
  File "${TIGCCDIR}\Lib\flashos.a"
  ...
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\GCC4TI Documentation.lnk" "$INSTDIR\Doc\tigcc.chm"
  !insertmacro MUI_STARTMENU_WRITE_END
SectionEnd

Section "GCC4TI IDE" SEC02
  SetOutPath "$INSTDIR\Bin"
  File "${TIGCCDIR}\Bin\ide.exe"
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
  CreateDirectory "$SMPROGRAMS\$ICONS_GROUP"
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\GCC4TI IDE.lnk" "$INSTDIR\Bin\ide.exe"
  !insertmacro MUI_STARTMENU_WRITE_END
...
Section "Command Line Compiler (TIGCC.EXE)" SEC03
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
  CreateDirectory "$SMPROGRAMS\$ICONS_GROUP"
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\GCC4TI Command Line Prompt.lnk" "command.com"
  !insertmacro MUI_STARTMENU_WRITE_END
...
Section -AdditionalIcons
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\Uninstall GCC4TI.lnk" "$INSTDIR\uninst.exe"
  !insertmacro MUI_STARTMENU_WRITE_END
SectionEnd
...
Section -Post
  ...
  WriteRegStr HKLM "Software\GCC4TI Team\GCC4TI" "Program Folder" "$INSTDIR"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC01} "Components required to use GCC4TI"
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC02} "Integrated development environment"
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC03} "Command-line interface to GCC4TI"
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC04} "Examples can help you to understand GCC4TI's functionality better. You can use the GCC4TI IDE to open the example projects."
...
have_inst_dir:
  ...
  Delete "$SMPROGRAMS\$ICONS_GROUP\GCC4TI Command Line Prompt.pif"
  Delete "$SMPROGRAMS\$ICONS_GROUP\GCC4TI Documentation.lnk"
  Delete "$SMPROGRAMS\$ICONS_GROUP\GCC4TI IDE.lnk"
  Delete "$SMPROGRAMS\$ICONS_GROUP\Uninstall GCC4TI.lnk"

  RMDir "$SMPROGRAMS\$ICONS_GROUP"

  Delete "$0\Bin\as.exe"
  Delete "$0\Bin\m68k-coff-tigcc-as.exe"
  Delete "$0\Bin\cc1.exe"
  Delete "$0\Bin\gcc.exe"
  Delete "$0\Bin\m68k-coff-tigcc-gcc.exe"
  Delete "$0\Bin\m68k-coff-tigcc-nm.exe"
  Delete "$0\Bin\m68k-coff-tigcc-objcopy.exe"
  Delete "$0\Bin\m68k-coff-tigcc-objdump.exe"
  Delete "$0\Bin\m68k-coff-tigcc-size.exe"
  Delete "$0\Bin\m68k-coff-tigcc-strip.exe"
  Delete "$0\Bin\ar-tigcc.exe"
  Delete "$0\Bin\ld-tigcc.exe"
  Delete "$0\Bin\link.dll"
  Delete "$0\Bin\tigcc-patcher.exe"
  Delete "$0\Bin\ide.exe"
  Delete "$0\Bin\tigcc.exe"
  Delete "$0\Bin\tprbuilder.exe"
  Delete "$0\Bin\templates.dat"
  Delete "$0\Bin\a68k.exe"
  Delete "$0\Bin\pack.exe"
  Delete "$0\Bin\ttarchive.exe"
  Delete "$0\Bin\ttbin2bin.exe"
  Delete "$0\Bin\ttbin2hex.exe"
  Delete "$0\Bin\ttbin2oth.exe"
  Delete "$0\Bin\ttbin2str.exe"
  Delete "$0\Bin\ttchecksum.exe"
  Delete "$0\Bin\ttextract.exe"
  Delete "$0\Bin\tthelp.exe"
  Delete "$0\Bin\tthex2bin.exe"
  Delete "$0\Bin\ttinfo.exe"
  Delete "$0\Bin\ttpack.exe"
  Delete "$0\Bin\ttppggen.exe"
  Delete "$0\Bin\ttsetname.exe"
  Delete "$0\Bin\ttsplit.exe"
  Delete "$0\Bin\ttstrip.exe"
  Delete "$0\Bin\tttiler.exe"
  Delete "$0\Bin\ttunarchive.exe"
  Delete "$0\Bin\ttunpack.exe"
  RmDir "$0\Bin"
  Delete "$0\Lib\tigcc.a"
  Delete "$0\Lib\flashos.a"
